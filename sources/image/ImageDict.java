...
public class ImageDict implements ImageGrise {
	...
	private Dictionnaire<Point,NiveauGris> dico;
	
	public ImageDict(int largeur, int hauteur) {
		this.largeur = largeur;
		this.hauteur = hauteur;
		dico = new TabDict<Point,NiveauGris>();
	}
    ...
    public NiveauGris pointEn(int x, int y) {
    	if (dico.contientClef(new Point(x, y))) {
    		return dico.valeurPour(new Point(x,y));
    	}
    ...
    public void definirPoint(int x, int y, NiveauGris gris) {
    	if (gris != NiveauGris.BLANC) {
    		dico.ajouter(new Point(x, y), gris);
    	}
    }
    
    /** Met en noir le point de coordonnées (x,y) */
    public void allumer(int x, int y) {
    	while (!dico.valeurPour(new Point(x, y)).estNoir()) {
    		definirPoint(x, y, dico.valeurPour(new Point(x, y)));
    	}
    }
    
    /** Met en blanc le point de coordonnées (x,y) */
    public void eteindre(int x, int y) {
    	dico.enleverPour(new Point(x,y));
    }
    ...
    public void randomize() {
    	...
    	for (int i = 0; i < largeur; i++) {
    		for (int j = 0; j < hauteur; j++) {
    			if ((grisTmp = NiveauGris.randomize()) != NiveauGris.BLANC) {
    				definirPoint(i, j, grisTmp);
    			}
    			else {
    				eteindre(i, j);
    			}
